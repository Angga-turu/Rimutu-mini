...
async function startBot() {
  ...
  // Check if sessionID is provided and process RimuruMini! format session
  if (config.sessionID && config.sessionID.startsWith('RimuruMini!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'RimuruMini' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'RimuruMini!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from RimuruMini Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing RimuruMini session:', e.message);
